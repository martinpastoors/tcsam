...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER_VECTOR(logVarLogCatch);  // Log variance of log catch
  PARAMETER_VECTOR(logQ);     // Log of catchability parameters
  PARAMETER_VECTOR(logVarLogSurvey); // Lo variance of the log survey 
  ...
  for(int i=0; i<logObs.size(); ++i){
    ...
    if(fleet(i)==1){
      ...
      sd = exp(Type(0.5)*logVarLogCatch[a]);
    }else{
      pred = logQ(a)-(F(a,y)+M(a,y))*surveyTime+logN(a,y);
      sd = exp(Type(0.5)*logVarLogSurvey(y));
    }    
